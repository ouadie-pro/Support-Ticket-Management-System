...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Réclamation {{ $complaint->numero }}</h1>
            <p class="text-gray-500 mt-1">Soumise le {{ $complaint->created_at->format('d/m/Y à H:i') }}</p>
        </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
                    <h3 class="text-lg font-semibold text-gray-900">{{ $complaint->sujet }}</h3>
                </div>
                <div class="p-6">
                    <div class="prose max-w-none text-gray-700 whitespace-pre-wrap">{{ $complaint->description }}</div>
                    ...
                    <div class="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
                        ...
                        <p class="text-green-700 whitespace-pre-wrap">{{ $complaint->resolution }}</p>
            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden">
                ...
                <div class="p-6 max-h-96 overflow-y-auto space-y-4" id="messages-container">
                    @forelse($complaint->comments as $comment)
                    <div class="flex {{ $comment->utilisateur_id === auth()->id() ? 'justify-end' : 'justify-start' }}">
                        <div class="max-w-[80%] rounded-lg p-4 {{ $comment->utilisateur_id === auth()->id() ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-900' }}">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <span class="text-xs opacity-75">{{ $comment->created_at->format('d/m/Y H:i') }}</span>
                            </div>
                            <p class="whitespace-pre-wrap">{{ $comment->contenu }}</p>
                        </div>
                    </div>
                    @empty
                    ...
                    <form method="POST" action="{{ route('complaints.comments.store', $complaint->id) }}" id="reply-form">
                        ...
                        <div class="flex gap-3">
                            <textarea name="contenu" id="complaint-contenu" placeholder="Écrire une réponse..." rows="2" required
                                class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 resize-none"></textarea>
                            <button type="submit" class="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition self-end">
            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <p class="font-medium text-gray-900">{{ $complaint->agent->name ?? 'Non assigné' }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="font-medium text-gray-900">{{ $complaint->date_resolution->format('d/m/Y à H:i') }}</p>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <form method="POST" action="{{ route('complaints.assign', $complaint->id) }}">
                        ...
                        <select name="traite_par" class="w-full px-3 py-2 border border-gray-300 rounded-lg mb-2">
                    ...
                    <form method="POST" action="{{ route('complaints.update-priority', $complaint->id) }}">
                        ...
                        <select name="priorite" class="w-full px-3 py-2 border border-gray-300 rounded-lg mb-2">
                    ...
                    <form method="POST" action="{{ route('complaints.update-status', $complaint->id) }}">
                        ...
                        <select name="statut" class="w-full px-3 py-2 border border-gray-300 rounded-lg mb-2">
